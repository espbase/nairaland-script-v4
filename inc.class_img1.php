...
####################################################################################################################
   ...
   // for ($i = 0; $i < count($_FILES['file']['name']); $i++) {//loop to get individual element from the array
      ...
      if (($_FILES["file1"]["size"] < 1024*5120) //Approx. 5MB files can be uploaded.
            ...
            if (move_uploaded_file($_FILES['file1']['tmp_name'], $target_path.$imgurl)) {//if file moved to uploads folder
                // save to databse
     $mtk = $db->query("UPDATE class_img SET class_img1='$imgurl' WHERE class_fk='$id' ");
